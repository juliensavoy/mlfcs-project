...
class tabular_dataset(Dataset):
    def __init__(
        ...
        if dataset_name == 'ihdp':
            dataset_path = "./data_ihdp/ihdp_norm_data/" + current_id + ".csv"
            print('dataset_path', dataset_path)
            processed_data_path = (
                f"./data_ihdp/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data_ihdp/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))
        ...
        if dataset_name == 'jobs':
            ...
            print('dataset_path', dataset_path)
            processed_data_path = (
                f"./data_jobs/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data_jobs/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))
        ...
        if dataset_name == 'twins':
            ...
            print('dataset_path', dataset_path)
            processed_data_path = (
                f"./data_twins/missing_ratio-{missing_ratio}_seed-{seed}.pk"
            )
            processed_data_path_norm = (
                f"./data_twins/missing_ratio-{missing_ratio}_seed-{seed}_max-min_norm.pk"
            )
            os.system('rm {}'.format(processed_data_path))
...
def get_dataloader(seed=1, nfold=5, batch_size=16, missing_ratio=0.1, dataset_name = 'acic2018', current_id='0'):
    ...
    if dataset_name == 'acic2016':
        ...
        valid_index = remain_index[: int(tsi*0.1)] # validation is the first 10% of train
    ...
    if dataset_name == 'acic2018':
        ...
        np.random.shuffle(remain_index) # shuffled
        num_train = (int)(len(remain_index) * 1)

        train_index = remain_index[: tsi] 
        valid_index = remain_index[: int(tsi*0.1)]  # a subset of training set. 10%.
    ...
    if dataset_name == 'ihdp':
        ...
        processed_data_path_norm = (
            f"./data_ihdp/missing_ratio-{missing_ratio}_seed-{seed}_current_id-{current_id}_max-min_norm.pk"
        )
        print(
            "------------- Perform data normalization and store the mean value of each column.--------------"
        )
